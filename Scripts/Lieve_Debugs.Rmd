...
# remove id's that are not present in the groundbeeltes_community
species_habitat_filtered <- species_habitat %>%
  semi_join(groundbeetle_community, by = "id")
glimpse(species_habitat)

species_habitat_breed <- species_habitat %>%
  filter(description != "0") %>%           # Eerst: verwijder "0"-waardes
  pivot_wider(
    names_from = type,                     # Maak van de 'type'-waarden kolomnamen
    values_from = description,             # Vul de kolommen met 'description'
    values_fn = list(description = ~ first(.))  # In geval van meerdere, neem de eerste
  )
